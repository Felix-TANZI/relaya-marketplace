...
def _customer_notifications_enabled() -> bool:
    """Relu a chaque etape : couper l'assistant ne demande pas de redemarrage."""
    from .conf import get_config

    config = get_config()
    return config.enabled and config.customer_notifications
...
class WhatsAppAssistantConfig(AppConfig):
    ...
    def ready(self):
        ...
        from .bridge.orders import connect_shipment_signal
        ...
        from .conversation.customer import notify_step
        ...
        connect_shipment_signal(notify_step, _customer_notifications_enabled)
