...
const CATEGORIES: Record<ParcelSize, {
  nomKey: string; exempleKey: string; icone: (c: string) => React.ReactNode;
}> = {
  SMALL: { nomKey: "rl2_parcel_size.category_small_name", exempleKey: "rl2_parcel_size.category_small_example",
    icone: (c) => <Mail size={22} color={c} /> },
  STANDARD: { nomKey: "rl2_parcel_size.category_standard_name", exempleKey: "rl2_parcel_size.category_standard_example",
    icone: (c) => <Package size={22} color={c} /> },
  LARGE: { nomKey: "rl2_parcel_size.category_large_name", exempleKey: "rl2_parcel_size.category_large_example",
    icone: (c) => <Box size={22} color={c} /> },
  BULKY: { nomKey: "rl2_parcel_size.category_bulky_name", exempleKey: "rl2_parcel_size.category_bulky_example",
    ...
    return (
      <div style={cadre}>
        <p style={{ fontSize: 13, color: "#64748B", margin: 0 }}>
          {t("rl2_parcel_size.determining_category")}
        </p>
      </div>
    );
  }
    ...
    return (
      <div style={{ ...cadre, background: "#FFF7ED", borderColor: "#FED7AA" }}>
        <p style={{ fontSize: 13, color: "#9A3412", margin: 0, lineHeight: 1.6 }}>
          {t("rl2_parcel_size.category_undetermined")}
        </p>
      </div>
    );
  }
    ...
    <div>
      <div style={{
        ...
        <span style={{ fontSize: 13, color: "#334155", fontWeight: 700 }}>
          {t("rl2_parcel_size.category_heading")}
        </span>
        <span style={{
          ...
          <Lock size={12} />
          {t("rl2_parcel_size.determined_by_belivay")}
        </span>
      </div>
      ...
      <div style={{
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <p style={{
            ...
            {refuse
              ? t("rl2_parcel_size.contract_excludes_category")
              : t("rl2_parcel_size.example_with_reason", { example: t(cat.exempleKey), reason: suggestion.reason })}
          </p>
        </div>
        ...
        {!refuse && tarif && (
          <div style={{ textAlign: "right" }}>
            ...
            <p style={{ fontSize: 11, margin: "2px 0 0", color: "#64748B" }}>
              {t("rl2_parcel_size.fcfa_for_this_parcel")}
            </p>
          </div>
        )}
      </div>
      ...
      {!suggestion.confident && (
        <div style={{
          ...
          <p style={{ fontSize: 12.5, color: "#9A3412", margin: 0, lineHeight: 1.55 }}>
            {t("rl2_parcel_size.uncertain_deduction_notice")}
          </p>
        </div>
      )}
      ...
      {refuse && (
        <div style={{
          ...
          <p style={{ fontSize: 12.5, color: "#7F1D1D", margin: 0, lineHeight: 1.55 }}>
            {t("rl2_parcel_size.refused_category_notice")}
          </p>
        </div>
      )}
    </div>
