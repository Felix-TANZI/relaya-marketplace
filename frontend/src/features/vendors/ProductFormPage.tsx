...
import { BrandAutocomplete } from "@/features/catalog/BrandAutocomplete";
import {
  AttributeInput,
  type AttributeValue,
} from "@/features/catalog/AttributeInput";

import type { CategoryTreeNode } from "@/services/api/categories";
import type { BrandLight } from "@/services/api/brands";
import {
  attributesApi,
  type MasterProductAxes,
} from "@/services/api/attributes";
import { variantsApi, type AxisValues } from "@/services/api/variants";
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          background: accent ? T.orangeL : T.cream,
          borderBottom: `1px solid ${accent ? T.orangeB : T.border}`,
        }}
      >
        <div
  ...
  return (
    <div className="space-y-1.5">
      <label
        ...
        style={{ color: error ? T.red : T.text }}
      >
        {label}
      ...
      {children}
      {error && (
        <p className="text-[11px] font-semibold" style={{ color: T.red }}>
          {error}
        </p>
      )}
      {!error && hint && (
...
function defaultCampaignForm(
  ...
  return {
    ...
    promoPrice:
      referencePrice > 0 ? String(Math.round(referencePrice * 0.8)) : "",
    stockReserved: "5",
  };
}
...
export default function ProductFormPage() {
  ...
  const [masterMode, setMasterMode] = useState<"new" | "existing">(
    id ? "new" : "existing",
  );
  const [masterQuery, setMasterQuery] = useState("");
  const [masterResults, setMasterResults] = useState<MasterFiche[]>([]);
  const [selectedMaster, setSelectedMaster] = useState<MasterFiche | null>(
    null,
  );
  ...
  const [selectedBrand, setSelectedBrand] = useState<BrandLight | null>(null);
  const [masterAxes, setMasterAxes] = useState<MasterProductAxes | null>(null);
  const [axisValues, setAxisValues] = useState<Record<string, AttributeValue>>(
    {},
  );
  const [variantResolutionError, setVariantResolutionError] = useState<
    string | null
  >(null);
        ...
        if (isEdit && id) {
          ...
          if (pCat) {
            try {
              ...
              const node = findNode(tree, pCatId);
          ...
          for (const av of product.attribute_values ||
            ([] as ProductAttributeSelection[]))
            ea[av.attribute.id] = av.selected_values;
          setAttrVals(ea);
        }
      } catch {
  ...
  useEffect(() => {
    ...
    vendorsApi
      .getProductAttributes(parseInt(effectiveCatId))
      .then(setAttributes)
      .catch(() => setAttributes([]));
  }, [effectiveCatId]);*/
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    attributesApi
      .getMasterAxes(selectedMaster.slug)
      .then((axes) => {
        if (cancelled) return;
        setMasterAxes(axes);
        // Reset des valeurs à chaque changement de master
        setAxisValues({});
      })
      .catch(() => {
        if (cancelled) return;
  ...
  const campaignDiscountPct =
    campaignReference > campaignPromo && campaignPromo > 0
      ? Math.round((1 - campaignPromo / campaignReference) * 100)
      : 0;
  const campaignDurationHours =
    campaignForm.startsAt && campaignForm.endsAt
      ? (new Date(campaignForm.endsAt).getTime() -
          new Date(campaignForm.startsAt).getTime()) /
        3_600_000
      : 0;
  ...
  const updateCampaignForm = <K extends keyof CampaignForm>(
    key: K,
  ...
  const validate = (): boolean => {
    ...
    if (requestCampaign) {
      const starts = campaignForm.startsAt
        ? new Date(campaignForm.startsAt)
        : null;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Résolution du variant (Electronics + master avec axes) ──
      ...
      if (needsVariantResolution) {
        // Vérifier que tous les axes ont une valeur
        const missingAxes = masterAxes.variant_axes_resolved
          .filter((axis) => {
            const v = axisValues[axis.slug];
            return (
              v === null ||
              v === undefined ||
              v === "" ||
        ...
        // Construction du dict axis_values
        const axisValuesPayload: AxisValues = {};
        for (const axis of masterAxes.variant_axes_resolved) {
          const v = axisValues[axis.slug];
          if (v && typeof v === "object" && "id" in v) {
            axisValuesPayload[axis.slug] = v.name;
          } else if (v !== null && v !== undefined) {
            axisValuesPayload[axis.slug] = v as string | number;
          }
        }

        // Appel find-or-create
        try {
          setVariantResolutionError(null);
          const result = await variantsApi.findOrCreate({
            master: selectedMaster.id,
            axis_values: axisValuesPayload,
          });
          resolvedVariantId = result.variant.id;
      ...
      const payload: ProductPayload = {
        ...
        brand_fk:
          !isEdit && masterMode === "new" && selectedBrand
            ? selectedBrand.id
            : null,
                      ...
                      {masterResults.map((m) => (
                        <button
                          ...
                          onClick={() => {
                            setSelectedMaster(m);
                            setMasterResults([]);
                            setMasterQuery(m.title);
                          }}
                          className="w-full text-left px-3 py-2 text-[12.5px] transition-all"
                          style={{
                            background:
                              selectedMaster?.id === m.id ? T.orangeL : T.white,
                            color: T.text,
                            borderBottom: `1px solid ${T.border}`,
                          }}
                        >
                          <span className="font-semibold">{m.title}</span>
                          <span style={{ color: T.mutedL }}>
                            {" "}
                            · {m.category_name}
                            {m.brand ? ` · ${m.brand}` : ""}
                          </span>
                  ...
                  {masterAxes.variant_axes_resolved.map((axis) => (
                    <AttributeInput
                      ...
                      attribute={{
                        ...
                        role: "AXE",
                        values_type: axis.values_type,
                        values: axis.values,
                        unit: axis.unit,
                        is_universal: axis.is_universal,
                        category: null,
                        category_name: null,
                        display_order: 0,
                      }}
                      value={axisValues[axis.slug] ?? null}
                      onChange={(v) =>
                        setAxisValues((prev) => ({ ...prev, [axis.slug]: v }))
                      }
                      required
                    />
                  ))}
                  ...
                  <textarea
                    ...
                    style={
                      errors.description
                        ...
                        : { ...iBase, resize: "vertical", minHeight: 120 }
                    }
                  />
                </Field>
                          ...
                          {attr.values.map((val) => {
                            ...
                            return (
                              <button
                                key={val}
                                type="button"
                                onClick={() => toggleAttr(attr.id, val)}
                                className="px-3 py-1.5 rounded-xl text-[12.5px] font-semibold border-2 transition-all"
                                style={{
                                  background: sel ? T.orangeL : T.cream,
                                  borderColor: sel ? T.orange : T.border,
        ...
        <div className="space-y-5">
          ...
          <Section title="Prix & Stock" icon={<DollarSign size={15} />} accent>
            ...
            >
              ...
              {discPct > 0 && (
                <div
                  className="flex items-center gap-2 mt-1.5 px-3 py-2 rounded-xl"
                  style={{
                    background: T.orangeL,
                    border: `1px solid ${T.orangeB}`,
                  }}
              ...
              <div className="space-y-4">
                  ...
                  {[
                    ...
                    return (
                      <button
                        ...
                        type="button"
                        onClick={() =>
                          updateCampaignForm("campaignType", option.value)
                        }
                ...
                >
                  <input
                    ...
                    onChange={(e) =>
                      updateCampaignForm("title", e.target.value)
                    }
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <Field label="Début" required error={errors.campaignStartsAt}>
                    <input
                      ...
                      onChange={(e) =>
                        updateCampaignForm("startsAt", e.target.value)
                      }
                  ...
                  >
                    <input
                      ...
                      onChange={(e) =>
                        updateCampaignForm("endsAt", e.target.value)
                      }
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  >
                    <div className="relative">
                      <input
                        type="number"
                        value={campaignForm.referencePrice}
                        onChange={(e) =>
                          updateCampaignForm("referencePrice", e.target.value)
                        }
                        min={price || 100}
                        style={{
                          ...(errors.campaignReferencePrice ? iErr : iBase),
                          paddingRight: 46,
                        }}
                      />
                      <span
                  ...
                  >
                    <div className="relative">
                      <input
                        type="number"
                        value={campaignForm.promoPrice}
                        onChange={(e) =>
                          updateCampaignForm("promoPrice", e.target.value)
                        }
                        min={100}
                        style={{
                          ...(errors.campaignPromoPrice ? iErr : iBase),
                          paddingRight: 46,
                        }}
                      />
                      <span
                ...
                {campaignForm.campaignType === "FLASH" && (
                  ...
                  >
                    <input
                      type="number"
                      value={campaignForm.stockReserved}
                      onChange={(e) =>
                        updateCampaignForm("stockReserved", e.target.value)
                      }
                      min={5}
