 ...
 */
export const VENDOR_ALLOWED_TRANSITIONS: Partial<
...
export const vendorsApi = {
  ...
  uploadImage: async (
    ...
    return http<ProductImage>(`/api/vendors/products/${productId}/images/`, {
      method: "POST",
      body: formData,
      // ATTENTION : ne PAS mettre 'Content-Type': 'multipart/form-data' manuellement.
      // Le navigateur l'ajoute automatiquement avec le boundary. Le forcer casse l'upload.
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
  ...
  setPrimaryImage: async (
    ...
    const token = localStorage.getItem("access_token");
    return http<ProductImage>(
      `/api/vendors/products/${productId}/images/${imageId}/set-primary/`,
      {
        method: "PATCH",
        headers: { Authorization: `Bearer ${token}` },
      },
  ...
  getOrders: async (filters?: VendorOrderFilters): Promise<VendorOrder[]> => {
    ...
    if (filters?.escrow_status)
      params.append("escrow_status", filters.escrow_status);
  ...
  updateFulfillmentStatus: async (
    ...
    return http<VendorOrder>(
      `/api/vendors/orders/${orderId}/fulfillment-status/`,
      {
        method: "PATCH",
        body: JSON.stringify(data),
        headers: {
  ...
  cancelWithdrawal: async (
    ...
    const token = localStorage.getItem("access_token");
    return http<WithdrawalRequest>(
      `/api/vendors/withdrawals/${withdrawalId}/cancel/`,
      {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
      },
  ...
  getProductAttributes: async (
    ...
    const token = localStorage.getItem("access_token");
    return http<ProductAttribute[]>(
      `/api/vendors/products/attributes/?category=${categoryId}`,
      {
        headers: { Authorization: `Bearer ${token}` },
      },
    );
  ...
  duplicateProduct: async (
    ...
    const token = localStorage.getItem("access_token");
    return http<VendorProductEnriched>(
      `/api/vendors/products/${productId}/duplicate/`,
      {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
      },
  ...
  getDisputes: async (
    filter?: "urgent" | "mediation" | "closed",
  ): Promise<VendorDisputeListItem[]> => {
    const token = localStorage.getItem("access_token");
   ...
   */
  submitDisputeReply: async (
    disputeId: number,
    data: VendorDisputeReplyPayload,
  ): Promise<VendorDisputeDetail> => {
    const token = localStorage.getItem("access_token");
    return http<VendorDisputeDetail>(
      `/api/vendors/disputes/${disputeId}/reply/`,
      {
        method: "POST",
        body: JSON.stringify(data),
        headers: {
  ...
  sendDisputeMessage: async (
    disputeId: number,
    message: string,
  ): Promise<VendorDisputeMessage> => {
    const token = localStorage.getItem("access_token");
    return http<VendorDisputeMessage>(
      `/api/vendors/disputes/${disputeId}/messages/`,
      {
        method: "POST",
  ...
  getShopQr: async (): Promise<{
    slug: string;
    public_url: string;
    shop_name: string;
  }> => {
    const token = localStorage.getItem("access_token");
    return http("/api/vendors/shop/qr/", {
      headers: { Authorization: `Bearer ${token}` },
    });
  },
  ...
  getSubscriptionHistory: async () => {
    const token = localStorage.getItem("access_token");
    return http("/api/vendors/plans/history/", {
      headers: { Authorization: `Bearer ${token}` },
    });
  },
